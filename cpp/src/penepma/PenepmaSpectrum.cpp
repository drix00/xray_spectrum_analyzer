 ...
 * @file
 *
 * @brief Implementation to read penepma spectrum file.
 *
...
PenepmaSpectrum::PenepmaSpectrum(const boost::filesystem::path &filepath) : dataFilepath(filepath)
{
    energies_eV.reserve(1000);
    intensities_1_eVsre.reserve(1000);
    intensityErrors_1_eVsre.reserve(1000);
}

PenepmaSpectrum::~PenepmaSpectrum()
{
    energies_eV.clear();
    intensities_1_eVsre.clear();
    intensityErrors_1_eVsre.clear();
}

Energies_eV PenepmaSpectrum::getEnergies_eV()
{
...
Intensities_1_eVsre PenepmaSpectrum::getIntensities_1_eVsre()
{
...
Intensities_1_eVsre PenepmaSpectrum::getIntensityErrors_1_eVsre()
{
    if (intensityErrors_1_eVsre.empty())
    {
        readData();
    }

    return intensityErrors_1_eVsre;
}

void PenepmaSpectrum::readData()
{
    ...
    while(inputFile.good())
    {
        ...
        if (!line.empty())
        {
            ...
            if(splitVector.empty() || splitVector.size() < 3)
            {
                continue;
            }
            ...
            if (splitVector.size() == 3)
            {
                ...
                const double intensityError_1_eVsre = extractDoubleValueFromString(splitVector[2]);

                energies_eV.push_back(energy_eV);
                intensities_1_eVsre.push_back(intensity_1_eVsre);
                intensityErrors_1_eVsre.push_back(intensityError_1_eVsre);
            }
        }
    }
